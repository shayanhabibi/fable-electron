...
module ApiDocs =
    ...
    let validateDir () =
        if 
            ...
            |> not
        then Directory.create VirtualRoot.fsdocs.``.``
...
module Laundry =
    ...
    let withGithubClient = fun func ->
        let token = Args.gitClientToken |> Option.defaultWith(fun _ -> Environment.environVarOrFail "GITHUB_TOKEN")
...
module Electron =
    ...
    let getReleases () =
        Electron.getReleases()
    let tryGetRelease (predicate: ReleaseInfo -> bool) = Electron.getReleases() |> List.tryFind predicate
    let downloadRelease =
        Electron.downloadElectronApi  apiFile
        >> Async.RunSynchronously
    let tryGetReleaseFromString input =
        Electron.getReleases()
        |> List.tryFind (_.tagName.Trim('v') >> (=) input)
    let private cacheFile = Root.ci.``cache.json``
    /// Fallback for not being able to parse version in changelog
    let tryGetCachedRelease () =
        try
            File.readAsString cacheFile
    ...
    let writeToCache (releaseInfo: ReleaseInfo) =
        ...
        |> File.writeString false cacheFile
    let generate () =
    ...
    let installTests cleanInstall =
        if cleanInstall
        then Npm.cleanInstall testDir
        else Npm.install testDir
    let test () =
        Npm.test testDir
    let isDirty () =
